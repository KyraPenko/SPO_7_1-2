    ...
    public static void execute(ArrayList<Pair<String, String>> tokens) {
        ...
        for(int i = 0; i < tokens.size(); i++)
        {
            ...
            else if(token.getFirst().equals("VAR_TYPE"))
            {
                ...
                if(token.getSecond().equals("int"))
                {
                    int_vars.put(variable.getSecond(), 0);
                }
                else if(token.getSecond().equals("float"))
                {
                    float_vars.put(variable.getSecond(), 0.0f);
                }
                else if(token.getSecond().equals("bool"))
                {
                    bool_vars.put(variable.getSecond(), false);
                }
                else if(token.getSecond().equals("list"))
                {
                    list_vars.put(variable.getSecond(), new LList<Integer>());
                }
                else if(token.getSecond().equals("set"))
                {
                    set_vars.put(variable.getSecond(), new HSet<Integer>());
                }
                stack.add(variable);
            }
            else if(token.getFirst().equals("PRINT_LK"))
            {
